...
public class JsTokenFilterNeeded implements ContainerRequestFilter {
    ...
    public ContainerRequest filter(ContainerRequest request) {
        System.out.println(request.getHeaderValue("username"));
        System.out.println(request.getHeaderValue("privatekey"));
        String path = request.getPath();
        if (path.equals(AUTHORIZATION_SERVICE_PATH) || request.getMethod().equals("OPTIONS"))
            return request;
